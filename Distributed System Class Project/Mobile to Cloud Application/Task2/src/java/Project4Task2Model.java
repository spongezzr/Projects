...
/**
 * This class serves as the Model in Project 4 Task 2. It calls the third-party
...
public class Project4Task2Model {
    ...
    // The MongoDBClient to set up connection with database
    private MongoDBClient dbClient = new MongoDBClient();
    // The recorder that records data in each request
    private Document dataRecorder;

    /**
     * When a new request comes, it reset the data recorder.
     */
    public void initializeRecorder() {
        dataRecorder = new Document();
    }

    /**
     * A getter method.
     *
     * @return the current data recorder
     */
    public Document getRecorder() {
        return dataRecorder;
    }
     ...
     * A getter method.
     *
     * @return the current MongoDBClient
     */
    public MongoDBClient getDBClient() {
        return dbClient;
    }
    ...
    public JSONArray search(String searchParam) {
        ...
        try {
            ...
            String url = BASE_URL + "?" + searchParam + query;

            // record the destination url to TicketMaster
            dataRecorder.append("api_request_url", url);
            ...
            * In order to clearly read the events, we convert the response to a JSONArray.
            ...
            // record how many events are returned by TicketMaster
            dataRecorder.append("api_response_length", events.length());
    ...
    private String getAddress(Object addressJSON) throws JSONException {
        ...
        String city = ((JSONObject) venue.get("city")).get("name").toString();
        return city;
    }

    /**
     * A method that will be called by the servlet. It record all useful data
     * from the request and response
     *
     * @param request current HttpServletRequest
     * @param response current HttpServletResponse
     * @param destination here it is "client", but can be various endpoint in
     * future implementation
     */
    public void record(HttpServletRequest request, HttpServletResponse response, String destination) {
        dataRecorder.append(destination + "_request_host", request.getHeader("Host"));
        dataRecorder.append(destination + "_connection", request.getHeader("Connection"));
        dataRecorder.append(destination + "_response_code", response.getStatus());
    }
}
